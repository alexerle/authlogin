  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.status === 429) {
        setError('Zu viele Versuche. Bitte warten Sie einige Minuten und versuchen Sie es erneut.')
      } else if (err.response?.data?.message?.includes('already exists')) {
        setError('Diese E-Mail-Adresse ist bereits registriert')
      } else {
        setError(err.response?.data?.message || err.message || 'Registrierung fehlgeschlagen')
      }
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-950">
        <p className="font-semibold">Ihr erstes Konto wird als Kundenadministrator angelegt.</p>
        <p className="mt-1">
          Verwenden Sie bitte Ihre persönliche geschäftliche E-Mail-Adresse und keine Sammeladresse wie
          info@. Weitere Mitarbeiter können Sie nach der Freischaltung selbst anlegen und verwalten.
        </p>
      </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="auth-input pl-11 pr-10"
              disabled={isLoading}
